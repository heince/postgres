 ...
 *
 * IDENTIFICATION
 *    contrib/passwordcheck_custom/passwordcheck_custom.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
/* custom by ITG */
#define MIN_HURUF_BESAR 3
#define MIN_HURUF_KECIL 3
#define MIN_ANGKA 3
               ...
               bool validuntil_null)
{
    ...
    /* custom by ITG */
    int huruf_besar, huruf_kecil, angka;
    huruf_besar = huruf_kecil = angka = 0;
    ...
    switch (password_type)
    {
        ...
        case PASSWORD_TYPE_PLAINTEXT:
            ...
            if (angka < MIN_ANGKA)
            {
                ereport(ERROR,
                        (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                errmsg("password policy minimum digit not satisfied")));
            }

            if (huruf_besar < MIN_HURUF_BESAR)
            {
                ereport(ERROR,
                        (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                errmsg("password policy minimum capital letter not satisfied")));
            }

            if (huruf_kecil < MIN_HURUF_KECIL)
            {
                ereport(ERROR,
                        (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                errmsg("password policy minimum small letter not satisfied")));
            }
